...
class Trainer(object):
    ...
    def load_checkpoint(
        ...
        elif getattr(self.cfg.dataset,'load_from_which_ckpt', None) is not None:
            if self.cfg.dataset.load_from_which_ckpt.endswith('.pt'):
                f_checkpoint_path = self.cfg.dataset.load_from_which_ckpt
            else: 
                f_checkpoint_path = os.path.join(self.cfg.dataset.load_from_which_ckpt, "checkpoint_best.pt") 
            with open(f_checkpoint_path, "rb") as f:
                f_state = torch.load(f, map_location=torch.device("cpu"))
            model_f_dict = self.get_model().state_dict()

            base_dict = {k:v for k,v in f_state['model'].items() if k in model_f_dict}
            model_f_dict.update(base_dict)
            self.get_model().load_state_dict(model_f_dict) 
            logger.info(">>> successful load from provided ckpt and start to train.")
        else:
