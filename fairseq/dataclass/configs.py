...
class DatasetConfig(FairseqDataclass):
    ...
    load_from_which_ckpt: Optional[str] = field(
        default=None, metadata={"help": "enable to load and init model from given ckpt."}
    )
    xlmr_task: Optional[str] = field(
        default='fixall', metadata={"help": "how to incorporate xlmr into NMT model."}
    )
    MPLM_type: Optional[str] = field(
        default='xlmr', metadata={"help": "type of multilingual pretrained model, xlmr is XLM-R base, xlmrL is XLM-R Large."}
    )
    ViT_type: Optional[str] = field(
        default='clip', metadata={"help": "Type of vision transformer. clip is CLIP base, swinT is swin Transformer"}
    )
    loss_type: Optional[str] = field(
        default='sim', metadata={"help": "Type of loss used."}
    )
    truncate_target: bool = field(
        default=False, metadata={"help": "disable validation"}
    )
    xlmr_modeldir: Optional[str] = field(
        default='xlmr_base', metadata={"help": 'perform task related to xlmr'}
    )
    enable_lang_proj: bool = field(default=False,metadata={
        'help': 'if True, do not add lang linear layer to encoder output'})
