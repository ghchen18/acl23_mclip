...
def _upgrade_state_dict(state):
    ...
    if "optimizer_history" not in state and 'optimizer' in state:
        best_loss = state["best_loss"] if 'best_loss' in state else 10

        state["optimizer_history"] = [
            {"criterion_name": "CrossEntropyCriterion", "best_loss": best_loss}
        ]
