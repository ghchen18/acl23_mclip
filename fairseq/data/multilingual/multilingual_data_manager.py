...
import fairseq.data.image_caption_dataset as imca_data
...
class MultilingualDatasetManager(object):
    def __init__(self, args, lang_pairs, langs, dicts, sampling_method, truncate_source=False, truncate_target=False, cfg=None):
        super().__init__()
        self.cfg = cfg
        self.args = args
        self.seed = args.seed
        ...
        self.args.same_lang_per_batch = True 
        self.args.enable_lang_ids = True 
        self.enable_add_xlmr_bos = getattr(self.args, 'enable_add_xlmr_bos')
        self.truncate_target = getattr(self.args, 'truncate_target')
        self.mscoco_dir = getattr(self.args, 'mscoco_dir', None)

        if self.mscoco_dir is None:
            self.train_with_image_caption = False 
            self.mask_idx = None
        else:
            self.train_with_image_caption = True
        
            try:
                self.mask_idx = self.dicts['im'].add_symbol("<mask>")
            except:
                self.dicts['im'] = self.dicts['en']
                self.mask_idx = self.dicts['en'].add_symbol("<mask>")


    @classmethod
    def setup_data_manager(cls, args, lang_pairs, langs, dicts, sampling_method, cfg=None):
        return MultilingualDatasetManager(
            args, lang_pairs, langs, dicts, sampling_method, cfg=cfg
        )
    ...
    def add_args(parser):
        ...
        parser.add_argument('--mscoco-dir', default=None, type=str,
                            help='path to image features, every image has one numpy(.np) file. ')
        parser.add_argument('--enable-add-xlmr-bos', action="store_true", default=False,
                            help='path to source dictionary; if specified it will override per language dictionary loading')
        parser.add_argument('--read-raw-images', action="store_true", default=False,
                            help='path to source dictionary; if specified it will override per language dictionary loading')
        parser.add_argument('--lambda-config', default="0.0", type=str, metavar='CONFIG',
        ...
        try:
            parser.add_argument('--mask-prob', default=0.15, type=float,
                                help='for masked lm task.')
            parser.add_argument('--leave-unmasked-prob', default=0.1, type=float, 
                                help='for masked lm task.')
            parser.add_argument('--random-token-prob', default=0.1, type=float, 
                                help='for masked lm task.')                    
            parser.add_argument('--freq-weighted-replacement', action="store_true", default=False, 
                                help='for masked lm task.')
            parser.add_argument('--mask-whole-words', action="store_true", default=False, 
                                help='for masked lm task.')
            parser.add_argument('--mask-multiple-length', default=1, type=int, 
                                help='for masked lm task.')
            parser.add_argument('--mask-stdev', default=0, type=float,
                                help='for masked lm task.')
        except:
            pass 
    ...
    def _shared_collater(self):
        return not (self.args.extra_data and "mono_dae" in self.args.extra_data and "denoised" in self.args.extra_data and "mlm" in self.args.extra_data ) and (
        ...
        for lang in langs_to_load_dicts:
            if args.fixed_dictionary is not None:
                ...
                augment_dictionary(
                    ...
                    add_langtok_into_dict = not getattr(args, 'enable_add_xlmr_bos'),
                    use_xlmr_dict_style = getattr(args, 'enable_add_xlmr_bos'),
                )
    ...
    def load_lang_dataset(
        ...
        if self.enable_add_xlmr_bos:
            assert hasattr(src_dict, "bos_index") 
            src_dataset = PrependTokenDataset(src_dataset, src_dict.bos())
            logger.info(f">>> Now Add BOS to src dataset of {src} - {tgt} tokens ...")
        elif prepend_bos:
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        if src == 'im': 
            image_ids_file = os.path.join(self.args.data, f'{split}-ids.{tgt}.raw.txt')
            captions_file = os.path.join(data_path, f'{split}.{tgt}-None.{tgt}')

            with open(image_ids_file, 'r') as f:
                image_ids = [line.strip('\n').split()[0] for line in f]

            captions_ds = data_utils.load_indexed_dataset(captions_file, tgt_dict)
            if self.truncate_target:
                captions_ds = AppendTokenDataset(
                    TruncateDataset(
                        StripTokenDataset(captions_ds, tgt_dict.eos()),
                        77 - 1,
                    ),
                    tgt_dict.eos(),
                )

            langpair_ds = imca_data.ImageCaptionDataset(
                    image_ids, split, cap_ds=captions_ds, cap_dict=tgt_dict, 
                    cfg=self.cfg, src_lang_id = _lang_id(lang_dictionary, src_lang),
                    tgt_lang_id = _lang_id(lang_dictionary, tgt_lang), tgt_lang=tgt,
                )
            ...
            langpair_ds = self.load_langpair_dataset(
                ...
                src_lang_id=_lang_id(lang_dictionary, src_lang)
                if enable_lang_ids and lang_dictionary is not None
                else None,
                tgt_lang_id=_lang_id(lang_dictionary, tgt_lang)
                if enable_lang_ids and lang_dictionary is not None
                else None,
    ...
    def get_data_paths_and_lang_pairs(self, split):
        if self.args.langtoks is None or len(self.args.langtoks) == 0:
            datapaths = {"main": self.args.data}
            lang_pairs = {"main": self.lang_pairs}
        else:
            key_length = len(list(self.args.langtoks.keys()))
            if key_length > 1:
                datapaths = {key: self.args.data for key in list(self.args.langtoks.keys()) }
                lang_pairs = {key: self.lang_pairs for key in list(self.args.langtoks.keys())}
            else:
    ...
    def get_split_num_data_shards(self, split):
        ...
        for data_category, paths in data_paths.items():
            ...
            for src, tgt in lang_dirs:
                ...
                if "mono_" in data_category or 'denoised' in data_category or 'mlm' in data_category:
        ...
        if self.train_with_image_caption:
            for key, value in lang_pairs.items(): 
                for val in value:
                    src, tgt = val.split('-')
                    if src == 'im':
                        num_shards_dict[f'main:im-{tgt}'] = 1 
    ...
    def get_shard_id(cls, num_shards, epoch, shard_epoch=None):
        ...
        shard = (shard - 1) % num_shards if num_shards > 0 else 0
        return shard
    ...
    def get_split_data_param_list(self, split, epoch, shard_epoch=None):
        ...
        for data_category, paths in data_paths.items():
            ...
            for src, tgt in lang_dirs:
                assert src is not None or data_category in ['mono_dae','denoised', 'mlm'], (
                ...
                try:
                    ...
                    param_list.append(
                        {
                            ...
                            if src and data_category not in ['mono_dae','denoised','mlm']
                            else None,
                            "tgt": tgt,
                    ...
                    logger.info(f"Error: Not found data files for {data_category}: {src} - {tgt}")
        return param_list
    ...
    def get_train_sampling_ratios(
        ...
        if sampling_func is None:
            sample_ratios = None 
        elif len(data_sizes) == 1:
            sample_ratios = sampling_func(data_sizes[0])
        else:
            mx = []
            for item in data_sizes:
                mx = mx + item
            sample_ratios = sampling_func(mx)
    ...
    def load_split_datasets(
        ...
        datasets = []
        for param in data_param_list:
            mds = self.load_a_dataset(combine=combine, langpairs_sharing_datasets=langpairs_sharing_datasets, **param)
            if mds is not None:
                datasets.append((param["key"], mds))
